const { createGoods } = require("../../services/goods");
...
Page({
  ...
  handleShelfLifeTypeChange(event) {
    ...
    const shelfLifeType =
      this.data.shelfLifeOptions[shelfLifeIndex] || this.data.shelfLifeOptions[0];
  ...
  async handleSubmit() {
    ...
    try {
      ...
      const errorMessage =
        error && error.data && error.data.message === "goods limit reached"
          ? "每个用户最多只能上架7件商品"
          : "发布失败，请重试";
